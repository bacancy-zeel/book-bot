...
1. **One document per book, not fixed-size chunks.** A book row is already a
   self-contained record, so no text splitter is used here. Splitting on a
   token window would cut one blurb in half and glue it to an unrelated book,
   so "a novel about a desert planet" would retrieve fragments instead of
   books.

2. **Resumable by construction.** Each book gets a deterministic id and is
   written with that id, so re-running skips what is already stored instead of
   starting over. An interrupted run costs you nothing.
"""
...
def build_document(row: pd.Series) -> Document:
    """Return the Document whose page_content is embedded and whose metadata
    rides along unembedded.

    Only meaning-bearing prose goes into page_content. Publication year,
    rating, ids and language codes are stored as metadata but deliberately kept
    OUT of the vector: they are identical boilerplate on every record, so they
    pull all the vectors toward a common centroid and wash out the signal that
    actually distinguishes one book from another.

    It matters more than it looks, because the embedding model truncates at a
    few hundred tokens. Thirty tokens of "Published: / Average rating: / Book
    id:" at the front push that much of the blurb straight out of the window.
    """
    ...
    page_content = ". ".join(p for p in [lead, description] if p)
...
def ingest(path: str | None = None, limit: int | None = None,
    ...
    vectors = store.vectorstore(reset=reset)
    ...
    total = len(ids)
    if total == 0:
        return {"added": 0, "skipped": len(frame), "total": store.count()}

    for start in range(0, total, config.EMBED_BATCH):
        stop = min(start + config.EMBED_BATCH, total)
        vectors.add_documents(documents[start:stop], ids=ids[start:stop])
        if progress:
            progress(stop, total)

    return {"added": total, "skipped": len(frame) - total, "total": store.count()}
